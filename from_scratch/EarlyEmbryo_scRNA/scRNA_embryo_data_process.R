...
#minor modifications by mackenzie noon, 2023
#mackenzie.noon@yale.edu
...
setwd('/home/mcn26/palmer_scratch/from_scratch/process/2_cellranger_make_matricies')
...
for (n in 1:11){ #Singleplexed embryos
  embryo_data_list[[n]]=Read10X(data.dir = paste0("Embryo",n,"/outs/filtered_feature_bc_matrix/"))
  print(n)
}

for (n in 12:15){ #Multiplexed embryos
  embryo_data_list[[n]]=Read10X(data.dir = paste0("MPlex",n-11,"/outs/filtered_feature_bc_matrix/"))
  print(n)
}
...
for (n in 12:15){
  clusters=read.table(paste0("MPlex",n-11,"/outs/analysis/clustering/gene_expression_graphclust/clusters.csv"),header=T)
...
p2 <- DimPlot(embryo.integrated, reduction = "umap", label = F, 
              repel = TRUE,cols=c("dodgerblue","olivedrab3","firebrick","darkviolet","orange"))
